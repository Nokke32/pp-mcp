...
nicht selbst parsen. Produktivbetrieb: Synology NAS "olten", intern im Docker-Netz
(nicht öffentlich exponiert, außer über nginx-proxy-manager unter `ppmcp.klobo.de`),
zusätzlich per `MCP_AUTH_TOKEN` geschützt.

## Betrieb & Entwicklung

Gedacht für Docker, läuft aber auch lokal. `docker-compose.yml` ist der
Multi-Source-Produktivbetrieb (NAS "olten", `MCP_AUTH_TOKEN` Pflicht);
...
`/Users/norbert/Applications/pp-mcp` (eigene Kopie, nicht dieses Repo). `switch-portfolio.sh`
